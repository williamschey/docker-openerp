...
def _call_config(arguments):
    """
    Sets confluence db details:
    <property name="hibernate.connection.password">os.environ["PGPASSWORD"]</property>
    <property name="hibernate.connection.url">arguments["--db"]</property>
    <property name="hibernate.connection.username">os.environ["PGUSER"]</property>
    """
    _exec(["sed", 's=\(connection.username">\).*<=\\1{PGUSER}<=g'.format(**os.environ), 
            "-i", os.path.join(os.environ["APP_VOLUME"], "confluence.cfg.xml")])
    _exec(["sed", 's=\(connection.password">\).*<=\\1{PGPASSWORD}<=g'.format(**os.environ), 
            "-i", os.path.join(os.environ["APP_VOLUME"], "confluence.cfg.xml")])
    _exec(["sed", 's=\(connection.url">\).*<=\\1{--db}<=g'.format(**arguments), 
            "-i", os.path.join(os.environ["APP_VOLUME"], "confluence.cfg.xml")])
...
def _call_boot(path):
    ...
    logfile = os.path.join(os.environ["APP_VOLUME"], "logs", "atlassian-confluence.log")
...
-d --db=<database_url>      Use a custom database [default: jdbc:postgresql://{PGHOST}:{PGPORT}/{PGDATABASE}]
