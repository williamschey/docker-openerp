...
def _call_boot(path):
    if os.environ["MYSQL_HOST"] == "localhost":
        _exec(['service', 'mysql', 'start'])
    _exec(['service', 'apache2', 'start'])
    _exec_fg(['tail', "-f", "/var/log/apache2/access.log"])
...
To develop or deploy using persistent storage:
    ...
    if arguments['show']:
        ...
        if arguments['shell']:
            _call_boot_shell(os.environ["APP_VOLUME"])
        else:
            _call_boot(os.environ["APP_VOLUME"])
    else:
        print('noop')
