...
def _call_boot(path):
    ...
    _exec_fg(['tail', "-f", "/var/log/postgresql/postgresql-{PGVERSION}-main.log".format(**os.environ)])
...
To develop or deploy using persistent storage:
    ...
    if arguments['show']:
        ...
        if arguments['shell']:
            _call_boot_shell(os.environ["DB_VOLUME"])
        else:
            _call_boot(os.environ["DB_VOLUME"])
    else:
        print('noop')
